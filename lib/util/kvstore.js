...
module.exports = function(prefix, Ravel) {
  ...
  kvstore.zadd = function(args, callback) {
    if (args.length < 3) {
      callback(new Ravel.ApplicationError.IllegalValue('ZADD requires at least 3 arguments.'), null);
    } else {
      args[0] = p(args[0]);
  ...
  kvstore.zrangebyscore = function(args, callback) {
    if (args.length < 3) {
      callback(new Ravel.ApplicationError.IllegalValue('ZRANGEBYSCORE requires at least 3 arguments.'), null);
    } else {
      args[0] = p(args[0]);
