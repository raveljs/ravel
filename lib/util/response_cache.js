...
class ResponseCacheMiddleware {
  ...
  get (key) {
    return new Promise((resolve, reject) => {
      this[sRavelInstance].kvstore.get(key, (err, result) => {
        ...
        if (err || result === null) { return reject(new Error(`Could not get cached response for key ${key}`)); }
        resolve(result);
      });
    });
  }
  ...
  async getFromCache (ctx, key, metaKey) {
    ...
    if (value) {
      ...
      ctx.response.etag = metadata.etag;
  ...
  async cacheBody (ctx, key, metaKey, options) {
    ...
    if (ctx.response.type) {
      await this.setex(metaKey, options.expire, JSON.stringify({
        type: ctx.response.type || 'text/html',
  ...
  /**
   * Computes a unique hash for a string.
   *
   * @param {string} str - A koa context.
   * @returns {string} A hash for the request url string.
   * @private
   */
  md5 (str) {
  ...
  middleware (inOptions = {}) {
    ...
    options = Object.assign(inOptions, options);
    return async (ctx, next) => {
      const key = 'ravel-cache-' + this.md5(ctx.request.url);
