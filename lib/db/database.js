  ...
  var middlewareTransaction = function() {
    return function(req, res, next) {
      ...
      async.series(starters, function(connectionErr, connections) {
        if (connectionErr) {
          Ravel.Log.e('Unable to start transaction.');
          Ravel.Log.e(connectionErr);
          ...
          var exitTransaction = function(shouldCommit, callback) {
            ...
            async.parallel(enders, function(endErr) {
              if (endErr) {
                Ravel.Log.e('Unable to end transaction.');
                Ravel.Log.e(endErr);
          ...
          var buildOverrideEnder = function(fToOverride) {
            return function(body) {
              if (req.transaction) {
                var shouldCommit = res.statusCode >= 200 && res.statusCode < 300;
          ...
          res.send = buildOverrideEnder(res.send);
          res.json = buildOverrideEnder(res.json);
          res.jsonp = buildOverrideEnder(res.jsonp);
    ...
    async.series(starters, function(connectionErr, connections) {
      if (connectionErr) {
        ...
        var exitTransaction = function(err, result) {
          var shouldCommit = err === undefined || err === null;
          for (var j=0;j<providers.length;j++) {
            enders.push(getEnder(providers[j], shouldCommit));
          }
          async.parallel(enders, function(endErr) {
