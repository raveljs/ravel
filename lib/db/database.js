...
module.exports = function(Ravel) {
  ...
  const providers = Ravel.get('database providers');
  ...
  const middlewareTransaction = function() {
    return function(req, res, next) {
      if (providers.length === 0) {
        Ravel.Log.debug('Middleware transaction attempted, but no database providers are registered.');
        req.transaction = {};
        next();
      } else {
        const starters = {},
  ...
  const scopedTransaction = function(onSuccess, actualCallback) {
    if (providers.length === 0) {
      Ravel.Log.debug('Scoped transaction attempted, but no database providers are registered.');
      onSuccess({}, function(){});
    } else {
      const starters = {},
            enders = [];
      ...
      async.series(starters, function(connectionErr, connections) {
        if (connectionErr) {
          ...
          const exitTransaction = function(err, result) {
            const shouldCommit = !Ravel.get('always rollback transactions') && (err === undefined || err === null);
            for (let j=0;j<providers.length;j++) {
              enders.push(getEnder(providers[j], shouldCommit));
            }
            async.parallel(enders, function(endErr) {
