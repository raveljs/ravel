...
class MiddlewareTransaction {
   ...
   */
  enter() {
    const ravelInstance = this._ravelInstance;
      ...
      if (providers.length === 0) {
        ...
        const getStarter = function(p) {
          return function(next2) {
            p.getTransactionConnection(next2);
          };
        };
        ...
        async.series(starters, function(connectionErr, connections) {
          if (connectionErr) {
            ...
            const getEnder = function(p, shouldCommit) {
              return function(next2) {
                p.exitTransaction(connections[p.name], shouldCommit, next2);
              };
            };
            ...
            //override all response-ending koa methods so that they close
            //the transaction
            ...
            //all koa response methods (send, json, jsonp) go through end()
...
class ScopedTransaction {
   ...
   *
   */
  enter(onSuccess, actualCallback) {
    const ravelInstance = this._ravelInstance;
    const providers = ravelInstance.get('database providers');
    if (providers.length === 0) {
      throw new ravelInstance.ApplicationError.IllegalValue(
        'Scoped transaction attempted, but no database providers are registered.');
    } else {
      const starters = {},
            enders = [];
...
module.exports = function(ravelInstance) {
  ...
  const middlewareTransaction = new MiddlewareTransaction(ravelInstance);
