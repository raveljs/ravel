...
module.exports = function(Ravel) { 
  var scope = {
    ...
    buildRestResponse: function(req, res, err, result, okCode, options) {
      ...
      if (err) {    
        if (err instanceof ApplicationError.NotFound) {
          res.status(scope.NOT_FOUND).end();
        } else if (err instanceof ApplicationError.Access) {
          res.status(scope.FORBIDDEN).end();
        } else if (err instanceof ApplicationError.NotAllowed) {
          res.status(scope.METHOD_NOT_ALLOWED).end();
        } else if (err instanceof ApplicationError.NotImplemented) {
          res.status(scope.NOT_IMPLEMENTED).end();
        } else if (err instanceof ApplicationError.DuplicateEntry) {
          res.status(scope.CONFLICT).end();
        } else if (err instanceof ApplicationError.RangeOutOfBounds) {
          res.status(scope.REQUESTED_RANGE_NOT_SATISFIABLE).end();
        } else if (err instanceof ApplicationError.IllegalValue) {
        ...
        totalCountArgs.push(function(err, count) {
          ...
          if (!range) {
            idArgs.push(
              ...
              1,
              function(err, result) {
                scope.buildRestResponse(lib, req, res, err, result);
              }
            );
            getFn.apply(this, idArgs);
          } else if (range === -1) {
            scope.buildRestResponse(lib, req, res, new ApplicationError.RangeOutOfBounds(), undefined);
          } else if (range === -2) {
            scope.buildRestResponse(lib, req, res, new ApplicationError.RangeOutOfBounds(), undefined);
          } else {
            idArgs.push(
              ...
              range[0].end,
              function(err, result) {
                scope.buildRestResponse(lib, req, res, err, result, rest.PARTIAL_CONTENT, {start:range[0].start, end:range[0].end, count:count});
              }
            );
            getFn.apply(this, idArgs);
          }      
