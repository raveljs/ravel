      ...
      if (req.headers['Range']) {
        ...
        totalCountArgs.push(function(err, count) {
          ...
          if (!range) {
            ...
            scope.buildRestResponse(req, res, new ApplicationError.RangeOutOfBounds(), undefined);
          } else if (range === -2) {
            scope.buildRestResponse(req, res, new ApplicationError.RangeOutOfBounds(), undefined);
          } else {
            idArgs.push(
              ...
              range[0].end,
              function(err, result) {
                scope.buildRestResponse(req, res, err, result, rest.PARTIAL_CONTENT, {start:range[0].start, end:range[0].end, count:count});
              }
            );
            getFn.apply(this, idArgs);
          }      
