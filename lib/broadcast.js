...
var l = require('./log')('broadcast');
var redis = require('redis');
var ApplicationError = require('./application_error');

var singleton = function(Ravel, primus, rooms) {
  if(singleton.caller !== singleton.getInstance){
    throw new Error('broadcast.js must be instantiated via getInstance(Ravel, primus)');
  }
  ...
  this.emit = function(room, event, data, dontCache) {
    if (rooms[room] === undefined) {
      l.w('Attempting to emit ' + data + ' to non-existent room \'' + room + '\'');
    }
    var time = Date.now();
    var msg = {
      event:event,
    ...
    if (!dontCache) {
      ...
      Ravel.kvstore.zadd([key, String(time), JSON.stringify(msg)], function(err, result) {
        if (err) {
          l.e(err);
        } else {
          //cleanup cache. TODO: move to daemon?
          Ravel.kvstore.zremrangebyscore(key, '-inf', String(time-cacheTimeSeconds), function(err, result) {
            if (err) {l.e(err);}
          });
        }
      }); 
 ...
 */
singleton.getInstance = function(Ravel, primus, rooms) {
  if(!this.instance) {
    this.instance = new singleton(Ravel, primus, rooms);
  }
