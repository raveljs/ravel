...
class AuthenticationMiddleware {
  ...
  /**
   * @returns {boolean} The value of redirect, as passed into the constructor. Useful for subclasses.
   * @private
   */
  get shouldRedirect () {
    ...
    return async function (ctx, next) {
      ...
      if (ctx.headers['x-auth-token'] && ctx.headers['x-auth-client']) {
        ...
        promise = Promise.reject(
          new self[sRavelInstance].$err.Authentication(
      ...
      try {
        ...
        if (!errorFromNext && self[sShouldRedirect]) {
          ...
          throw new self[sRavelInstance].$err.Authentication(
            `User with session id=${ctx.cookies.get('koa.sid')} is not authenticated`);
        } else {
