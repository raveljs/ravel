  ...
  var authorize = function(req, callback) {
    if (req.headers['x-auth-token'] && req.headers['x-auth-client']) {
      tokenAuth.tokenToProfile(req.headers['x-auth-token'], req.headers['x-auth-client'], function(err, profile) {
        if (err) {
          Ravel.Log.e('User not authorized to connect to primus');
          callback(err, false);
        } else {
          Ravel.Log.l('User id=' + profile.id + ' authorized to connect to primus via OAuth2.0 bearer token');
      ...
      expressSessionStore.get(cookie['connect.sid'], function(err, session) {
        ...
        if(!err && session && session.passport.user) {            
          Ravel.Log.l('User id=' + session.passport.user + ' authorized to connect to primus');
  ...
  primus.on('connection', function connection(spark) {
    ...
    spark.on('subscribe', function(data, callback) {      
      if (!data.room || typeof data.room !== 'string') {
        callback(Ravel.ApplicationError.IllegalValue('subscribe websocket message must contain \'room\' string property'), null);
        return;
      } 
      //try to resolve room string against list of registered room patterns 
      var resolvedRoom = RoomResolver.resolve(data.room);
      if (!resolvedRoom) {
        callback(Ravel.ApplicationError.NotFound('Specified room ' + data.room + ' does not exist.'), null);  
      } else {
        ...
        getUserId(spark, function(err, userId) {
          if (err) {
            ...
            throw new Ravel.ApplicationError.IllegalValue('Authorization function for room ' + data.room + ' is either undefined or not a function.');
          } else {
              ...
              if (err || !result) {
                ...
                spark.join(resolvedRoom.instance, function() {                  
                  ...
                  if (data.lastDisconnectTime) {
                    broadcast.getMissedMessages(resolvedRoom.instance, data.lastDisconnectTime, function(err, messages) {
                  ...
                  Ravel.Log.l('user id=' + userId + ' joined room ' + resolvedRoom.instance);
                  broadcast.emit(
    ...
    spark.on('unsubscribe', function(data, callback) {      
      if (!data.room || typeof data.room !== 'string') {
        callback(Ravel.ApplicationError.IllegalValue('unsubscribe websocket message must contain \'room\' string property'), null);
        return;
      }
      var resolvedRoom = RoomResolver.resolve(data.room);
      if (!resolvedRoom) {
        ...
        getUserId(spark, function(err, userId) {
          if (err || !userId) {
            ...
            spark.leave(resolvedRoom.instance, function() {
              ...
              callback(null, true);
              //let everyone know
              Ravel.Log.l('user id=' + userId + ' left room ' + resolvedRoom.instance);
              broadcast.emit(
                resolvedRoom.instance,
                'user disconnected',
                {userId:userId},
      ...
      if (!resolvedRoom) {
        ...
        getUserId(spark, function(err, userId) {
          if (err || !userId) {
            ...
            callback(new Ravel.ApplicationError.Access('User outside of room \'' + resolvedRoom.instance + '\' requested connected users.'), null);
          } else {
    ...
    getUserId(spark, function(err, userId) {
      ...
      function leaveRoom(room) {
        spark.leave(room, function() {
          ...
          //let everyone know
          Ravel.Log.l('user id=' + userId + ' left room ' + room);
          broadcast.emit(
            room,
            'user disconnected',
            {userId:userId},
