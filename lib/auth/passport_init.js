  ...
  ravelInstance.once('post module init', function() {
    ...
    if (providers.length > 0) {
      ...
      passport.deserializeUser(function(userId, done) {
        ravelInstance[symbols.authConfigModule].getUserById(userId)
        .then((user) => {
          done(null, user);
        }).catch((err) => {
          done(err, null);
      ...
      const verify = function(...verifyArgs) {
        const done = verifyArgs[verifyArgs.length-1];
        const args = verifyArgs.slice(0,verifyArgs.length-1);
        ravelInstance[symbols.authConfigModule].verifyCredentials(...args)
        .then((user) => {
          done(null, user);
        }).catch((err) => {
          done(err, null);
