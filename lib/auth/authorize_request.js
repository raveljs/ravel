...
module.exports = function authorizeRequest(Ravel, shouldRedirect, allowMobileRegistration) {
  ...
  const tokenAuth = require('./authorize_token')(Ravel);
    ...
    if (req.headers['x-auth-token'] && req.headers['x-auth-client']) {
      tokenAuth.tokenToProfile(req.headers['x-auth-token'], req.headers['x-auth-client'], function(err, profile) {
        if (err) {
          ...
          Ravel.get('get or create user function')(Ravel, undefined, undefined, profile, function(err2, user) {
            if (err2) {
              Ravel.Log.error(err2);
              fail(res);
              //next(err);
            } else {
              req.user = user;
          ...
          Ravel.get('get user function')(Ravel, profile, function(err2, user) {
            if (err2) {
              Ravel.Log.error(err2);
              fail(res);
              //next(err);
