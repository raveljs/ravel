...
class Ravel extends AsyncEventEmitter {
  ...
  constructor () {
    ...
    this.registerParameter('session secure', false, true);
    // Passport parameters
  ...
  /**
   * Helper method for building session options object.
   *
   * @type Object
   */
  buildSessionOptions () {
    const sessionOptions = {
      ...
      secure: true /* (boolean) secure or not (default true) */
    };
    if (this.get('session secure') === undefined) {
      sessionOptions.secure = undefined;
      const msg = "app.set('session secure', undefined) is NOT recommended and" +
      ' may open your application up to critical security vulnerabilities.' +
      ' Cookies sent over SSL are secure by default.';
      this.$log.warn(msg);
    } else if (this.get('session secure') === false && this.get('https') === false) {
      sessionOptions.secure = false;
      const msg = "app.set('session secure', false) is for development purposes only." +
      ' Unsetting session security is NOT recommended and opens your application' +
      ' up to critical security vulnerabilities.';
      this.$log.warn(msg);
    } else if (this.get('session secure') === false && this.get('https') === true) {
      const msg = "app.set('session secure', false) can not be used with https." +
      ' Session security defaulted to true.';
      this.$log.warn(msg);
    }
