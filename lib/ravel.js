...
class Ravel extends AsyncEventEmitter {
  ...
  async init () {
    ...
    if (this.get('koa favicon path')) {
      app.use(favicon(upath.toUnix(upath.posix.join(this.cwd, this.get('koa favicon path')))));
    }

    // static file serving
    if (this.get('koa public directory')) {
      const koaStatic = require('koa-static');
      const root = upath.toUnix(upath.posix.join(this.cwd, this.get('koa public directory')));
