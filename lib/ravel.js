...
class Ravel extends AsyncEventEmitter {
  ...
  async init () {
    ...
    app.use(session({
      ...
      secure: this.get('session secure') /* (boolean) secure or not (default true) */
    }, app));

    if (this.get('session secure') !== true) {
      this.$log.warn("app.set('session secure', false) is for development purposes only. Unsetting session security is NOT recommended and opens your application up to critical security vulnerabilities.")
    }
