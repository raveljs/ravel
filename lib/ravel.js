...
class Ravel extends EventEmitter {
  ...
  init() {
    ...
    const ExpressRedisStore = require('connect-redis')(session);
    ...
    const expressSessionStore = new ExpressRedisStore(sessionStoreArgs);
    ...
    if (Ravel.get('express view engine') && Ravel.get('express view directory')) {
      ...
      app.engine(Ravel.get('express view engine'),
        require(
          upath.join(Ravel.cwd, 'node_modules', Ravel.get('express view engine'))
        ).__express
      );
      //configure views
      app.set('views', upath.join(Ravel.cwd, Ravel.get('express view directory')));
    ...
    if (Ravel.get('express favicon path')) {
      app.use(favicon(upath.join(Ravel.cwd, Ravel.get('express favicon path'))));
    }
    ...
    if (Ravel.get('express public directory')) {
      app.use(express.static(upath.join(Ravel.cwd, Ravel.get('express public directory'))));
    }
    ...
    // const primus = new Primus(Ravel._server, { transformer: 'engine.io', parser: 'JSON' });
    // //primus_init produces a configured, cluster-ready broadcasting library
    // const broadcast = require('./ws/primus_init.js')(
    ...
    this._resourceInit(app);

    //create routes using factories
    this._routesInit(app);
