...
class Ravel extends AsyncEventEmitter {
  ...
  async init () {
    ...
    if (this.get('session secure') !== true) {
      this.$log.warn('Unsetting the session secure parameter is NOT recommended and opens application security vulnerabilities. This is only recommended for local development purposes.')
    }
