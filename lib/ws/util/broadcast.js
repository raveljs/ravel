...
let instance;
const Singleton = function(Ravel, primus, RoomResolver) {

  const broadcastChannel = 'tapestry_broadcast:';

  const cacheTimeSeconds = Ravel.get('websocket message cache time') ?
                           Number(Ravel.get('websocket message cache time')) : 24*60*60;

  const pub = redis.createClient(Ravel.get('redis port'), Ravel.get('redis host'), {});
  if (Ravel.get('redis password')) {
    pub.auth(Ravel.get('redis password'), function(err){if (err) {throw err;}});
  }
  pub.select(0);

  const sub = redis.createClient(Ravel.get('redis port'), Ravel.get('redis host'), {});
  if (Ravel.get('redis password')) {
  ...
  this.emit = function(room, event, data, dontCache) {
    if (RoomResolver.resolve(room) === undefined) {
      Ravel.Log.debug('Attempting to emit event \'' + event + '\' with data ' +
        data + ' to non-existent room \'' + room + '\'');
    } else {
      const time = Date.now();
      const msg = {
        event:event,
      ...
      if (!dontCache) {
        ...
        const key = 'ravel_broadcast_emit:'+room;
        Ravel.kvstore.zadd([key, String(time), JSON.stringify(msg)], function(err) {
          if (err) {
            Ravel.Log.error(err);
          } else {
            //cleanup cache. TODO: move to daemon?
            Ravel.kvstore.zremrangebyscore(key, '-inf', String(time-cacheTimeSeconds), function(err2) {
              if (err2) {Ravel.Log.error(err2);}
            });
          }
        });
      }
   ...
   */
   this.getMissedMessages = function(room, timestamp, callback) {
    if (Number(timestamp) < Date.now() - cacheTimeSeconds*1000) {
      ...
      callback(new ApplicationError.RangeOutOfBounds('Cannot satisfy request for missed messages because client lost ' +
        'connection outside of the cache history window. Please refresh all data.'), null);
    } else {
      ...
      Ravel.kvstore.zrangebyscore(['ravel_broadcast_emit:'+room, timestamp, '+inf'], function(err, messages) {
        if (err) {
          Ravel.Log.error(err);
          callback(new Error('Cannot satisfy request for missed messages due to error during cache access. ' +
            'Please refresh all data.'), null);
        } else {
