...
module.exports = function authorizeRequest(lib, kvstore, webFailureRedirect, allowMobileRegistration) {

  webFailureRedirect = webFailureRedirect || '/login';
    ...
    if (req.headers['x-auth-token'] && req.headers['x-auth-client']) {
      bearerAuth.bearerToProfile(req.headers["x-auth-token"], req.headers['x-auth-client'], function(err, profile) {
        if (err) {
          ...
          lib.users.getOrCreateUser(undefined, 'google', profile, function(err, user) {
            if (err) {
              next(err);
            } else {
              req.user = user;
          ...
          lib.users.getUserByProfile(undefined, 'google', profile, function(err, user) {
            if (err) {
              next(err);
            } else {
              req.user = user;
      ...
      ensureLoggedIn(webFailureRedirect)(req, res, next);
    }
  };
};
