...
module.exports = function(Ravel, primus, expressSessionStore) {
  ...
  var broadcast = require('./broadcast')(Ravel, primus);
  ...
  primus.on('connection', function connection(spark) {
    ...
    spark.on('subscribe', function(data, callback) {
      //check if the room is a registered room
      if (!data.room || typeof data.room !== 'string') {
        ...
        getUserId(spark, function(err, userId) {
          if (err) {
            callback(err, null);
          } else if (!rooms[data.room].authorize || typeof rooms[data.room].authorize !== 'function'){
            ...
            rooms[data.room].authorize(userId, function(err, result) {
              if (err || !result) {
                callback(err, null);
              } else {
                ...
                spark.join(data.room, function() {                  
                  ...
                  Ravel.kvstore.sadd(getRoomKey(data.room), userId);
                  //store new room in user's connected room list
                  Rave.kvstore.sadd(getUserKey(userId), data.room);
                  ...
                  l.l('user id=' + userId + ' joined room ' + data.room);
                  broadcast.emit(
                    data.room,
                    'user connected',
                    rooms[data.room].context,
                    {userId:userId},
                    true
                  );
                });
        ...
        getUserId(spark, function(err, userId) {
          if (err || !userId) {
            ...
            spark.leave(data.room, function() {
              ...
              Ravel.kvstore.srem(getRoomKey(data.room), userId);
              //remove room from user's connected room kvstore cache
              Ravel.kvstore.srem(getUserKey(userId), data.room);
              //callback immediately, we can do more cleanup after
              callback(null, true);
              //let everyone know
              l.l('user id=' + userId + ' left room ' + data.room);
              broadcast.emit(
                data.room,
                'user disconnected',
                rooms[data.room].context,
    ...
    spark.on('get connected users', function(data, callback) {
      if (!data.room || typeof data.room !== 'string') {
        ...
        getUserId(spark, function(err, userId) {
          if (err || !userId) {
            callback(err, null);
          } else if (!Ravel.kvstore.sismember(getRoomKey(data.room), userId)) {
            callback(new ApplicationError.Access('User outside of room \'' + data.room + '\' requested connected users.'), null);
          } else {
            Ravel.kvstore.smembers(data.room, callback);
          }
        });
      }
    });
        ...
        for (var i=0;i<connectedRooms.length;i++) {
          spark.leave(connectedRooms[i], function() {
            ...
            broadcast.emit(
              connectedRooms[i],
              'user disconnected',
              rooms[connectedRooms[i]].context,
              {userId:userId},
              true
            );
          });
