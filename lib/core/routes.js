...
const buildRoute = function (ravelInstance, routes, koaRouter, methodName, meta) {
  ...
  // we only need to check for method-level @cache if it isn't on the class
  if (Metadata.getClassMeta(routes, '@cache')) {
    const config = Metadata.getClassMetaValue(routes, '@cache', 'optionns', {});
    middleware.push(
      new ResponseCacheMiddleware(ravelInstance).middleware(config));
  } else if (Metadata.getMethodMeta(routes, methodName, '@cache')) {
 ...
 */
Routes.cache = require('./decorators/cache');
