...
module.exports = function (Ravel) {
  /**
   * Retrieve an initialized Ravel `Routes` module by its injection name, after app.init().
   *
   * @param {string} basePath - The basePath of the Routes module.
   */
  Ravel.prototype.routes = function (basePath) {
    if (!this.initialized) {
      throw new this.ApplicationError.General('Cannot retrieve a Routes reference from Ravel before app.init().');
    }
  ...
  Ravel.prototype[symbols.loadRoutes] = function (routesClass) {
    ...
    if (Metadata.getClassMetaValue(routesClass.prototype, '@role', 'type') !== 'Routes') {
      throw new this.ApplicationError.IllegalValue(`${routesClass.name} is not decorated with @Routes`);
    } else if (basePath === undefined) {
      throw new this.ApplicationError.IllegalValue(`basePath required for Routes class ${routesClass.name}`);
    }
