...
function buildRoute (ravelInstance, routes, koaRouter, methodName, meta) {
  ...
  for (let i = 0; i < toInject.length; i++) {
    const m = toInject[i];
    if (typeof m === 'object') {
      // handle parameterized middleware
      const [mName, mParams] = Object.entries(m)[0];
      const pmFactory = ravelInstance[symbols.injector].getModule(routes, mName);
      const pm = pmFactory(mParams);
      middleware.push(pm);
    } else {
