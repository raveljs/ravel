...
const AuthorizationMiddleware = require('../auth/authorize_request');
...
const buildRoute = function(ravelInstance, routes, koaRouter, methodName, meta) {
  ...
  if (Metadata.getClassMeta(routes, '@authenticated')) {
    ...
    middleware.push(
      new AuthorizationMiddleware(ravelInstance, config.shouldRedirect, config.allowRegistration).middleware);
...
// add in @authenticated decorator as static property
require('../auth/decorators/authenticated')(Routes);
