...
module.exports = function (Ravel) {
  ...
  Ravel.prototype[symbols.loadRoutes] = function (routesClass) {
    ...
    if (Metadata.getClassMetaValue(routesClass.prototype, '@role', 'type') !== 'Routes') {
      throw new this.$err.IllegalValue(`${routesClass.name} is not decorated with @Routes`);
    } else if (basePath === undefined) {
      throw new this.$err.IllegalValue(`basePath required for Routes class ${routesClass.name}`);
    }
