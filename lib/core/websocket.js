...
class Broker extends AsyncEventEmitter {
  ...
  init () {
    ...
    this.ravelApp.server.on('upgrade', (request, socket, head) => {
      this.wss.handleUpgrade(request, socket, head, (ws) => {
        // TODO removing client somewhere else would be more memory efficient.
   ...
   * @private
   * @param {string} topic - A dot-separated topic name, respecting /^\w+(?:\.\w+)*$/.
   * @param {Buffer} messageBuffer - A binary message buffer.
   */
  ...
  async localSubscribe (topic, clientID) {
    try {
      ...
      // tell elements of the app the subscription occurred
      await this.emit(`on subscribe to ${topic}`, clientID);
    } catch (err) {
      this.ravelApp.$log.debug(`Subscription to topic ${topic} failed for client ${clientID}`);
      this.ravelApp.$log.trace(err.stack);
    }
  }
  ...
  async localUnsubscribe (topic, clientID) {
    if (this.subscribers.has(topic)) {
      ...
      // tell elements of the app the subscription occurred
      await this.emit(`on unsubscribe from ${topic}`, clientID);
    }
  }
  ...
  localPublish (topic, messageBuffer) {
    for (const client of this.localSubscribers(topic)) {
      if (client.readyState !== WebSocket.OPEN) {
        // cleanup later
        setTimeout(() => this.localUnsubscribe(topic, client.cliendID), 1);
      } else {
