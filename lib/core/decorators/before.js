 ...
 *
 *   // &#64;mapping(Routes.GET, '/projects/:id')
 *   // &#64;before({'bodyParser': {onerror: (err, ctx) => ctx.throw('body parse error in handler', 422)}}) // parameters can be passed to middleware via parameter object
 *   async handler (ctx) {
 *     // in here, bodyParser will already have run,
 *     // and ctx.request.body will be populated
 *   }
  ...
  return function (target, key) {
    ...
    for (const elem of rest) {
      if (typeof elem === 'string') {
        ...
        if (Object.values(elem).filter(v => typeof v !== 'object').length > 0 ||
            Object.values(elem).length !== 1) {
          throw new $err.IllegalValue('Must supply a single value to @before decorator within objects.');
        }
        toInject.push(elem);
      } else {
