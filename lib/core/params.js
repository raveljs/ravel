...
module.exports = function(Ravel) {
   ...
   * This file must be located beside app.js
   * @memberof Ravel
   * @api private
   */
  Ravel.prototype[symbols.loadParameters] = function() {
    ...
    try {
      fromFile = require(upath.join(this.cwd, '.ravelrc.json'));
    } catch (err) {
      this.once('post init', () => {
        this.log.trace('Cannot locate .ravelrc.json file. This is not necessarily a bad thing, if one is not present.',
          err.stack);
      });
    }
