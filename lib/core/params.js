...
module.exports = function(Ravel) {
  ...
  Ravel.prototype[symbols.loadParameters] = function() {
    ...
    const path = this.cwd.split(upath.sep);
    path.unshift(upath.sep); // make sure we search root as well
    do {
      ...
      const search = upath.join(...path, '.ravelrc'); // don't require extension; node will search for .js/.json/.node
      try {
        fromFile = require(search);
        break;
      } catch (err) {
        this.once('post init', () => {
          this.log.trace(`Could not locate .ravelrc.json at ${search}`);
        });
        path.pop();
      }
    } while(path.length > 0);
