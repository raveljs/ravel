...
module.exports = function(Ravel, rooms) {
  ...
  Ravel.room = function(roomPattern, authorizationFunction) {
    ...
    if (rooms[roomPattern]) {
      throw new Ravel.ApplicationError.DuplicateEntry('Websocket room with path \'' + roomPattern + '\' has already been registered.');
    } else if (typeof authorizationFunction !== 'function') {
      throw new Ravel.ApplicationError.IllegalValue('Authorization function for path \'' + roomPattern + '\' must be a function.');
    }
    var params = [];
    ...
    Ravel.Log.i('Creating websocket room with pattern ' + roomPattern);
  };
 };