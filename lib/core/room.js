...
module.exports = function(Ravel, rooms) {
  const defaultAuthorizationFunction = function(userId, callback) {
    callback(null, true);
  };
  ...
  Ravel.room = function(roomPattern, authorizationFunction) {
    roomPattern = upath.normalize(roomPattern);
    ...
    const params = [];
    const paramMatcher = new RegExp(/\:(\w+)/g);
    let paramMatch = paramMatcher.exec(roomPattern);
