...
module.exports = function(Ravel) {
  ...
  Ravel.prototype.module = function(modulePath) {
    ...
    let moduleInject = require(upath.join(this.cwd, modulePath));

    if (typeof(moduleInject) === 'function' &&
      moduleInject.constructor !== undefined &&
      typeof moduleInject.constructor === 'function' &&
      moduleInject.constructor.name === 'Function'
    ) {
      //build injection function
      this._moduleFactories[name] = () => {
        //perform DI on module factory
        const temp = this._injector.inject({
          '$L': this.Log.getLogger(name),
          '$KV': this.kvstore,
          '$Params': {
            set: this.set,
            get: this.get,
      ...
      throw new this.ApplicationError.IllegalValue('Module with path ' +
        modulePath + ' must be an instantiable class.');
    }
