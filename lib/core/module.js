...
module.exports = function (Ravel) {
  ...
  Ravel.prototype.module = function (name) {
    if (!this.initialized) {
      throw new this.ApplicationError.General('Cannot retrieve a Module reference from Ravel before app.init().');
    }
  ...
  Ravel.prototype[symbols.loadModule] = function (moduleClass) {
    ...
    this[symbols.moduleFactories][name] = () => {
      ...
      const temp = this[symbols.injector].inject({
        '$app': this,
        '$err': this.ApplicationError,
        '$log': this.log.getLogger(name),
        ...
        '$db': {
          scoped: function (...args) {
            return this.db.scoped.bind(this.db)(...args);
          }
        }
      }, moduleClass);
