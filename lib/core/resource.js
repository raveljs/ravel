...
module.exports = function (Ravel) {
  ...
  Ravel.prototype.resource = function (basePath) {
    if (!this.initialized) {
      throw new this.ApplicationError.General('Cannot retrieve a Resource reference from Ravel before app.init().');
    }
  ...
  */
  Ravel.prototype[symbols.loadResource] = function (resourceClass) {
    ...
    if (Metadata.getClassMetaValue(resourceClass.prototype, '@role', 'type') !== 'Resource') {
      throw new this.ApplicationError.IllegalValue(`${resourceClass.name} is not decorated with @Resource`);
    } else if (basePath === undefined) {
      throw new this.ApplicationError.IllegalValue(`basePath required for Resource class ${resourceClass.name}`);
    }
 ...
 */
module.exports.mapping = function () {
