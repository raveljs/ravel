...
const buildRoute = function(ravelInstance, resource, app, methodType, methodName) {
  ...
  if (typeof resource[methodName] === 'function') {
    ...
    const middleware = [];
    let toInject = resource[methodName]();
    ...
    for (let i=0; i<toInject.length; i++) {
      const m = toInject[i];
      //the last element should be function middleware
      if (typeof m === 'function' && i === toInject.length-1) {
        middleware.push(m);
      //all the others should be strings
  ...
  static pre(...rest) {
    return function(target, key, descriptor) {
      if (rest.length === 0)  {
        ...
        target[`_original_${key}`] = value; // store original function for testing purposes
        rest.push(value);
