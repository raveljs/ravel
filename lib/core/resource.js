...
const buildRoute = function(ravelInstance, resource, koaRouter, methodType, methodName) {
  ...
  if (typeof resource[methodName] === 'function') {
    ...
    const middleware = [];
    let toInject = resource[utilSymbols.beforeMethodMiddleware] &&
                    resource[utilSymbols.beforeMethodMiddleware][methodName] ?
                      resource[utilSymbols.beforeMethodMiddleware][methodName] : [];
  ...
  constructor(basePath) {
    if (basePath === undefined) {
      throw new ApplicationError.IllegalValue(
        `Resource module \'${this.constructor.name}\' must call super(basePath)`);
    }
