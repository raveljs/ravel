  ...
  */
  Ravel.prototype[symbols.loadResource] = function (resourceClass) {
    ...
    if (Metadata.getClassMetaValue(resourceClass.prototype, '@role', 'type') !== 'Resource') {
      throw new this.$err.IllegalValue(`${resourceClass.name} is not decorated with @Resource`);
    } else if (basePath === undefined) {
      throw new this.$err.IllegalValue(`basePath required for Resource class ${resourceClass.name}`);
    }
